...
class GenerateController2 extends Controller
{
    ...
    public function inicio()
    {
        ...
        foreach (DB::getDoctrineSchemaManager()->listTables() as $table) {

            $tableName = $table->getName();

            $mapping[$tableName] = $this->rl_load_table_structure($tableName);
        }

        session(['mapping' => $mapping]);

        return view('rlustosa.first', compact('mapping'));
    }

    public function rl_load_table_structure($tableName): array
    {
        ...
        $mapping['class'] = Str::studly(Str::singular($tableName));
    ...
    public function mapa()
    {
        ...
        $tableData = $mapping[$table];

        $tableColumns = $tableData['columns'];
        $className = $tableData['class'];
        ...
        foreach ($this->fileTypes as $tipo) {
            ...
            $destino = str_replace('__CLASSE__', $className, $tipo['caminho']);
            $fileTypes[] = [
                'tipo' => $tipo['namespace'],
                'destino' => $destino,
        ...
        return view('rlustosa.mapa', compact('tableColumns', 'fileTypes', 'conversaoDeLabels', 'camposListagem', 'camposOcultos', 'camposCadastro', 'className', 'tableName'));
    }
    ...
    public function organizaVisual()
    {
        ...
        return view('rlustosa.organiza-visual', compact('campos_disponiveis', 'arquivos', 'className', 'tableName'));
    }
    ...
    public function gera()
    {
        ...
        //dd(request()->all(), $arquivos, $campos,$className, $tableName);
    ...
    public function gerarRules($camposDaTabela, $tabela, $nomeDaClasse)
    {
        ...
        foreach ($this->schemas as $schema) {
            ...
            if (is_array($schema) and count($schema)) {
                ...
                $mapping[$classeLaravel]['classe'] = $classeLaravel;

                $mapping[$classeLaravel]['tabela'] = $tabela;
            }
        }
        ...
        foreach ($camposDaTabela as $campo) {
            ...
            if ($campo['tipo'] === 'varchar') {
                ...
                if ($campo['key'] === 'primaria') {
                    ...
                    $regras[] = "'id' => 'required|integer|exists:" . $mapping[$nomeDaClasse]['tabela'] . ",id,deleted_at,NULL',";
